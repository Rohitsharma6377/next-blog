...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        const { title: [title], url: [url], category: [category], tags: [tags], author_id: [author_id], meta_title: [meta_title], meta_description: [meta_description], content: [content] } = fields;
        ...
        const results2 = await query(insertBlog, [title, url, content, author_id, mediaId]);
